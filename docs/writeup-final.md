...
**The model would not speak Ghana's languages, and lied about it convincingly.** Gemma 4 E2B writes good English. Asked for Ewe, Ga or Dagbani it returned fluent English under a local-language heading, and it collapsed Twi into a five-word phrase repeating until the token budget ran out. Our first repetition guard counted unique words in the tail, so a cycling *phrase* went straight through it.
